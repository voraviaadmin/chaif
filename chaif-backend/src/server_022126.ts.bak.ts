...
import { extractReceipt } from "./receipts/receiptOcr";
...
app.post("/v1/receipts/upload", requireVoraviaJwt, upload.single("file"), uploadReceiptHandler({ prisma, requireMember, audit }),async (req, res) => {
  try {
    // Resolve household context (supports dev override header)
    ...
    if (allowOverride && headerHouseholdId && headerHouseholdId.trim()) {
    ...
    if (!householdId) {
      return res.status(401).json({ error: "MISSING_householdId" });
    }
    ...
    if (!f?.buffer || !f?.mimetype) {
      return res.status(400).json({ error: "file is required (multipart field name: file)" });
          ...
          // ✅ allowlist mime types (images + pdf)
      ...
      if (!allowed.has(mime)) {
        return res.status(415).json({
          error: `Unsupported file type: ${mime}. Allowed: jpg/png/webp/pdf`,
        });
      }

      // ✅ basic size protection (in addition to multer limits)
      const maxBytes = Number(process.env.RECEIPT_MAX_UPLOAD_BYTES || String(12 * 1024 * 1024)); // 12MB default
      if (Number.isFinite(maxBytes) && f.size && f.size > maxBytes) {
 ...
 // ✅ OCR / PDF parse + deterministic parsing
 const extracted = await extractReceipt({ buffer: f.buffer, mimeType: mime });
    ...
    // Keep rawText optional. For privacy, you can disable it by env.
    ...
    const receipt = await prisma.receiptRaw.create({
      data: {
        ...
        lines: extracted.lines?.length
          ? {
            create: extracted.lines.map((l, idx) => ({
              lineNumber: idx + 1,
              rawLineText: l.rawLineText ?? null,
              vendorSku: l.vendorSku ?? null,
              barcode: l.barcode ?? null,
    ...
    const status = String(msg).includes("Unsupported file type") ? 415 : 500;
