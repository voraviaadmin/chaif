...
function parseReceiptTextDeterministic(
    ...
    const getRightMostMoney = (l: string): number | null => {
        const all = l.match(moneyTokenRe);
        if (!all?.length) return null;
        return safeParseNumber(all[all.length - 1]);
    };
    ...
    const looksLikeItemText = (l: string) => {
        // retailer agnostic: needs some letters OR a long sku-ish digit chunk + letters
        const letters = (l.match(/[A-Za-z]/g) ?? []).length;
        if (letters >= 3) return true;
        if (/^\d{5,}\s+[A-Za-z]/.test(l)) return true;
        return false;
    };

    function buildLogicalLinesRetailerAgnostic(rawLines: string[]) {
        const out: string[] = [];
        const pending: string[] = []; // FIFO queue of item-text-without-price
      
        for (const l of rawLines) {
          const lineTotal = getRightMostMoney(l);
      
          // Attach price-only lines to oldest pending item
          if (isPriceOnlyLine(l) && pending.length) {
            const item = pending.shift()!;
            out.push(`${item} ${l}`.trim());
            continue;
          }
      
          // If this is an item-looking line with no money → enqueue
          if (lineTotal === null && looksLikeItemText(l)) {
            pending.push(l);
            continue;
          }
      
          // Otherwise, flush any pending safely WITHOUT merging into this line
    ...
    const headerNoiseRe =
        /\b(orders\s*&\s*purchases|member|approved|purchase|instant\s+savings|thank\s*you|customer copy)\b/i;
...
export async function extractReceipt(file: {
    ...
    const normalizeProvider = (v: string): ReceiptOcrProviderName => {
    ...
    console.log("[receiptOcr] providerNormalized =", normalizeProvider(providerRaw));

    const provider = normalizeProvider(providerRaw);
    const fallback = normalizeProvider(fallbackRaw);
